#!/usr/bin/env python3
"""
Build lead-vanam-22-sep-data/leads.json and lead-vanam-22-sep-data/report.json
from Lead Vanam 22 Sep.xls, following the same conversion pattern as
lead-18-sep-data/build.py (_legacy preservation, _import_key idempotency,
_filled/_flags, duplicate groups) - the file has the exact same column
layout, so this is a near-copy adapted for a single-project (Vanam) batch.

old-data/ and lead-18-sep-data/ are READ ONLY here - used only as live
references for the JSON shape and project keys. Nothing there is written or
re-run; those batches are already imported.

Reads  Lead Vanam 22 Sep.xls          (read only)
       old-data/projects.json         (read only - key/name cross-check)
Writes lead-vanam-22-sep-data/leads.json
       lead-vanam-22-sep-data/duplicates.json
       lead-vanam-22-sep-data/report.json
...
Excel has no time-of-day column (created_at is a bare date string like
"9-22-26"), so every follow-up falls back to 10:00 AM Asia/Kolkata on the
fixed date 2026-09-23 - recorded per row in _follow_up_at /
_follow_up_time_source and aggregated in report.json. The import command
schedules each created open lead's single pending follow-up from those fields.

_import_key uses the "lead-vanam-22-sep:row-N" form throughout - deliberately
different in shape from old-data's "master_sheet:N" / "myco:N" /
"12_sep_lead:N" and from lead-18-sep's "lead-18-sep:row-N", so this batch's
idempotency can never collide with, or be confused with, an earlier one.

Standard library only except xlrd. Re-run with:
    python3 -B lead-vanam-22-sep-data/build.py
"""
...
XLS_PATH = os.path.join(ROOT, "Lead Vanam 22 Sep.xls")
OLD_DATA = os.path.join(ROOT, "old-data")  # read-only reference

SOURCE_FILE = "lead_vanam_22_sep"       # _source_file value on every row
IMPORT_KEY_PREFIX = "lead-vanam-22-sep"  # _import_key = "lead-vanam-22-sep:row-N"

# Fixed follow-up date from the brief: one pending follow-up per created open
# lead on 2026-09-23. Excel carries no time, so all rows use the fallback
# 10:00 AM Asia/Kolkata (applies to the whole lead, opening it for phone work).
FOLLOW_UP_DATE = "2026-09-23"
FOLLOW_UP_FALLBACK_TIME = "10:00:00"

# ---------------------------------------------------------------------------
# Stage mapping - the brief's table (the file only contains "Fresh", but the
# full known table is kept so an unexpected value still parses or fails loudly
# instead of silently guessing). Every target value already exists in
...
SOURCE_KEY = "facebook"  # all 33 rows

# stage -> role, mirroring config('crm.stage_owner_roles') /
# stage_owner_role_default: fresh -> telecaller.
...
# ---------------------------------------------------------------------------
# Decisions, carried over from the lead-18-sep batch decision the client
# confirmed on 2026-09-22 and re-confirmed for this file:
#
#   1. Assignment: every lead goes to the telecaller Riya Gandhi - which here
#      also happens to be what the file's own sales_person column says and
#      what stage_owner_roles would give (fresh -> telecaller). The file's
#      sales_person column is preserved in _legacy regardless.
...
#      already in the database - is dropped, not imported. Nothing is merged,
#      updated, or overwritten; the existing lead is left exactly as it is.
#
# Snapshot vs incremental: this file is an INCREMENTAL batch, not a snapshot.
# Its 33 rows postdate the old-data corpus already in the live database
# (dates 2026-09-13 .. 09-22); a live read (via tinker) shows exactly one
# (mobile, Vanam) pair already exists (#9408846346, stage lost, created
# 2026-05-08) plus other-project hits that do not collide because the unique
# index is (mobile_number, project_id). Expected outcome: 32 create,
# 1 skip_duplicate. See report.json's snapshot_analysis section.
# ---------------------------------------------------------------------------

# Live project ids (read-only resolve, checked by build, keyed like
# old-data/projects.json). 772 = Vanam.
PROJECT_ID_BY_KEY = {"vanam": 772}
...
def parse_created(value):
    """Excel created_at is a bare string like "9-22-26" (m-d-yy, no time)."""
    if isinstance(value, float):
        value = str(int(value))
    dt = datetime.strptime(str(value).strip(), "%m-%d-%y")
...
def check_projects_against_reference():
    """Cross-check Vanam against old-data/projects.json purely as a read-only
    sanity check - old-data is never written here."""
    projects = json.load(open(os.path.join(OLD_DATA, "projects.json"), encoding="utf-8"))
    by_key = {p["key"]: p["name"] for p in projects}
    if "vanam" not in by_key:
        raise SystemExit("reference check failed: old-data/projects.json has no 'vanam' project key")
    return by_key
...
def main():
    ...
    for r in range(1, sh.nrows):
        ...
        raw_rows.append({
            ...
            "created_at": parse_created(val(r, "created_at")),
        })

    assert len(raw_rows) == 33, f"expected 33 rows, got {len(raw_rows)}"
    ...
    for row in raw_rows:
        ...
        if project_key is None:
            raise SystemExit(f"row {row['_row_number']}: unrecognised project {project_raw!r} - "
                              f"does not match an existing project (Vanam)")
        ...
        lead = {
            ...
            "deleted_at": None,
            "_follow_up_at": f"{FOLLOW_UP_DATE} {FOLLOW_UP_FALLBACK_TIME}",
            "_follow_up_time_source": "fallback_excel_has_no_time_column",
            "_filled": filled,
            "_flags": flags,
    ...
    follow_up_counts = collections.Counter(l["_follow_up_time_source"] for l in leads)
    report = {
        "source_file": SOURCE_FILE,
        "import_key_prefix": IMPORT_KEY_PREFIX,
        "input_rows": len(raw_rows),
        "leads_out": len(leads),
        "snapshot_or_incremental": "incremental",
        "snapshot_analysis": {
            "summary": ("33-row batch postdating the already-imported old-data corpus "
                        "(dates 2026-09-13 .. 2026-09-22). Compared against old-data/leads.json, "
                        "lead-18-sep-data/leads.json and a live tinker read of leads: exactly one "
                        "(mobile, Vanam) pair already exists in the database (#9408846346, stage lost), "
                        "so 32 rows are new and 1 is a duplicate. No within-file duplicate mobiles."),
            "overlap_with_old_data_mobiles": sorted({l["mobile_number"] for l in leads}
                & {l["mobile_number"] for l in json.load(open(os.path.join(OLD_DATA, "leads.json"), encoding="utf-8"))
                   if l.get("project_key") == "vanam" and l.get("mobile_number")}),
            "overlap_with_lead_18_sep_mobiles": sorted({l["mobile_number"] for l in leads}
                & {l["mobile_number"] for l in json.load(open(os.path.join(ROOT, "lead-18-sep-data", "leads.json"), encoding="utf-8"))
                   if l.get("mobile_number")}),
        },
        "follow_up_date": FOLLOW_UP_DATE,
        "follow_up_time_source_counts": dict(follow_up_counts),
        "follow_ups_to_create": sum(1 for l in leads if l["_action"] == "create"
                                    and l["stage"] in ("fresh", "not_connected", "details_shared")),
