...
<template>
  <section class="min-w-0" aria-label="Channel partner chart">
    <div class="mb-3 flex flex-wrap items-center justify-between gap-3">
      <p class="rounded-lg bg-slate-100 dark:bg-slate-700 px-3 py-2 text-sm text-slate-700 dark:text-slate-300" role="status">
      ...
      <div class="flex flex-wrap items-center gap-3">
        ...
        <div class="flex overflow-hidden rounded-lg border border-slate-200 dark:border-slate-700" aria-label="Chart type">
          <button v-for="mode in ['bar', 'doughnut']" :key="mode" type="button"
                  class="px-2.5 py-1 text-xs font-medium capitalize transition"
                  :class="shape === mode ? 'bg-teal-700 text-white' : 'bg-white dark:bg-slate-800 text-slate-600 dark:text-slate-300 hover:bg-slate-50 dark:hover:bg-slate-700'"
                  :aria-pressed="shape === mode" @click="shape = mode">{{ mode }}</button>
        </div>
      </div>
    </div>
    <p v-if="!summary.rows.length" class="card p-8 text-center text-sm text-slate-500 dark:text-slate-400" role="status">
    ...
    <div v-else :style="{ '--partner-chart-height': `${height}px` }">
      ...
      <ul v-if="shape === 'doughnut'" class="mt-3 grid grid-cols-1 gap-2 text-xs text-slate-600 dark:text-slate-300 sm:grid-cols-2 lg:grid-cols-3" aria-label="Chart legend">
